...
## BTAB dashboard kit (default library)

Every visitor gets the **BTAB dashboard kit** in the Library panel: `public/btab-dashboard.excalidrawlib` (generated by `scripts/btab-kit/`) ships with the build, and `excalidraw-app/data/btabKit.ts` merges it in on load, after the IndexedDB library.

- Kit items own the `btab-kit:` id prefix. On every load all `btab-kit:*` items are replaced by the file's items; every other item (a user's own library) is kept. Regenerate + redeploy = everyone gets the new kit on their next load.
